...
///
/// Errors are communicated via a NULL return from `kreuzberg_render_pdf_pages`
/// plus `kreuzberg_last_error`. This struct is only returned on success.
#[repr(C)]
...
mod tests {
    ...
    fn test_c_render_result_size() {
        assert_eq!(
            ...
            16,
            "CRenderResult must be 16 bytes (ptr + usize)"
        );
    }
