...
fn flatten_json(text: &str) -> Result<Vec<Section>> {
    ...
    Ok(flatten_value_tree(
        mapping.iter().filter_map(|(k, v)| Some((k.as_str()?, v.clone()))),
    ))
}
...
fn flatten_value_tree<'a>(roots: impl Iterator<Item = (&'a str, serde_yaml_ng::Value)>) -> Vec<Section> {
    let mut sections: Vec<Section> = Vec::new();
    let mut queue: VecDeque<(Vec<String>, serde_yaml_ng::Value)> = VecDeque::new();

    for (key, val) in roots {
        queue.push_back((vec![key.to_string()], val));
    }
            ...
            Some(map) if !map.is_empty() => {
                for (k, v) in map {
                    if let Some(key_str) = k.as_str() {
                        ...
                        queue.push_back((child_path, v.clone()));
                    }
                }
            }
            _ => {
