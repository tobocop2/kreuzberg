...
fn render_render_test_c(fixture: &Fixture) -> Result<(String, String)> {
    ...
    match render.mode.as_str() {
        "single_page" => {
            ...
            writeln!(code, "    size_t png_len = 0;")?;
            writeln!(
                code,
                "    uint8_t *png_data = kreuzberg_render_pdf_page(document_path, {page_index}, {dpi_val}, &png_len);"
            )?;
            writeln!(code, "    if (!png_data) {{")?;
            writeln!(code, "        fprintf(stderr, \"FAIL: kreuzberg_render_pdf_page returned NULL\\n\");")?;
            writeln!(code, "        exit(1);")?;
            writeln!(code, "    }}")?;
            render_render_assertions_c(&assertions, "png_data", "png_len", &mut code)?;
            writeln!(code, "    free(png_data);")?;
        }
        "iterator" => {
            writeln!(
                code,
                "    KreuzbergPdfPageIterator *iter = kreuzberg_pdf_page_iterator_new(document_path, {dpi_val});"
            )?;
            ...
            writeln!(code, "    size_t page_len = 0;")?;
            writeln!(code, "    uint8_t *page_data = NULL;")?;
            writeln!(code, "    while ((page_data = kreuzberg_pdf_page_iterator_next(iter, &page_len)) != NULL) {{")?;
            writeln!(code, "        assert_is_png(page_data, page_len);")?;
            writeln!(code, "        free(page_data);")?;
