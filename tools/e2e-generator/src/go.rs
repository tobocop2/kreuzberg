...
fn render_render_test(fixture: &Fixture) -> Result<String> {
    ...
    writeln!(code, "        t.Skipf(\"Skipping {}: missing document\", {doc_path})", fixture.id)?;
    writeln!(code, "    }}")?;

    let dpi_arg = render
        .dpi
        .map(|d| format!(", kreuzberg.WithDPI({d})"))
        .unwrap_or_default();
    ...
    match render.mode.as_str() {
        "single_page" => {
            ...
            writeln!(
                code,
                "    pngData, err := kreuzberg.RenderPdfPage(documentPath, {page_index}{dpi_arg})"
            )?;
            writeln!(code, "    if err != nil {{")?;
        ...
        "iterator" => {
            writeln!(
                code,
                "    iter, err := kreuzberg.NewPdfPageIterator(documentPath{dpi_arg})"
            )?;
            writeln!(code, "    if err != nil {{")?;
            ...
            writeln!(code, "    for iter.Next() {{")?;
            writeln!(code, "        pngData := iter.Page()")?;
            writeln!(code, "        assertIsPng(t, pngData)")?;
            writeln!(code, "        pageCount++")?;
            writeln!(code, "    }}")?;
            writeln!(code, "    if err := iter.Err(); err != nil {{")?;
