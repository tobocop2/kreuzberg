...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path) -> Result<()> {
    ...
    if !render_fixtures.is_empty() {
        ...
        fs::write(tests_dir.join("RenderTest.php"), content)
...
fn render_render_category_php(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buffer)?;
    writeln!(buffer, "// Code generated by kreuzberg-e2e-generator. DO NOT EDIT.")?;
    writeln!(buffer, "// To regenerate: cargo run -p kreuzberg-e2e-generator -- generate --lang php")?;
    writeln!(buffer)?;
...
fn render_render_test_php(fixture: &Fixture) -> Result<String> {
    ...
    let method_name = format!("it_renders_{}", sanitize_identifier(&fixture.id));
    ...
    writeln!(code, "        if (!file_exists($documentPath)) {{")?;
    writeln!(code, "            $this->markTestSkipped('Missing document: ' . $documentPath);")?;
    writeln!(code, "        }}")?;
    ...
    match render.mode.as_str() {
        "single_page" => {
            ...
            writeln!(
                code,
                "        $pngData = Kreuzberg::render_pdf_page($documentPath, {page_index}{dpi_arg});"
            )?;
        ...
        "iterator" => {
            ...
            writeln!(
                code,
                "        foreach (Kreuzberg::render_pdf_pages_iter($documentPath{dpi_arg}) as $pngData) {{"
            )?;
...
fn render_render_assertions_php(assertions: &RenderAssertions, var: &str, code: &mut String, indent: &str) -> Result<()> {
