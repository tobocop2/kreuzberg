...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path) -> Result<()> {
    ...
    if !render_fixtures.is_empty() {
        ...
        fs::write(spec_dir.join("render_spec.rb"), content)
            .context("Failed to write Ruby render spec file")?;
    }
...
fn render_render_category_ruby(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buffer)?;
    writeln!(buffer, "# Code generated by kreuzberg-e2e-generator. DO NOT EDIT.")?;
    writeln!(buffer, "# To regenerate: cargo run -p kreuzberg-e2e-generator -- generate --lang ruby")?;
    writeln!(buffer)?;
...
fn render_render_example_ruby(fixture: &Fixture) -> Result<String> {
    ...
    match render.mode.as_str() {
        "single_page" => {
            ...
            writeln!(
                code,
                "    png_data = Kreuzberg.render_pdf_page(document_path.to_s, {page_index}{dpi_arg})"
            )?;
        ...
        "iterator" => {
            ...
            writeln!(
                code,
                "    Kreuzberg.render_pdf_pages_iter(document_path.to_s{dpi_arg}) do |png_data|"
            )?;
            ...
            if let Some(page_count_gte) = assertions.page_count_gte {
                writeln!(
                    code,
                    "    expect(pages.length).to be >= {page_count_gte}"
                )?;
            }
        }
