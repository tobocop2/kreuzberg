...
fn render_render_tests(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buffer, "use kreuzberg::pdf::{{render_pdf_page_to_png, PdfPageIterator}};")?;
    writeln!(buffer)?;
...
fn render_render_test(fixture: &Fixture) -> Result<String> {
    ...
    match render.mode.as_str() {
        "single_page" => {
            ...
            writeln!(
                test_body,
                "    let png_data = render_pdf_page_to_png(&document_path, {page_index}, {dpi})\n        .expect(\"Failed to render PDF page\");\n"
            )?;
        ...
        "iterator" => {
            writeln!(
                test_body,
                "    let iterator = PdfPageIterator::new(&document_path, {dpi})\n        .expect(\"Failed to create PDF page iterator\");"
            )?;
            writeln!(test_body, "    let mut page_count = 0usize;")?;
            writeln!(test_body, "    for png_data in iterator {{")?;
            writeln!(
                test_body,
...
fn render_render_assertions_rust(assertions: &RenderAssertions, var: &str, buf: &mut String) -> Result<()> {
    if assertions.is_png == Some(true) {
        writeln!(
            buf,
            "    assert!({var}.len() >= 4, \"Data too short for PNG\");"
        )?;
        writeln!(
            buf,
