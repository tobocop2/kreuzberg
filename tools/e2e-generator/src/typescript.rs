...
fn render_render_category_ts(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buffer, "import {{ existsSync }} from \"node:fs\";")?;
    writeln!(buffer, "import {{ renderPdfPageSync, JsPdfPageIterator }} from \"@kreuzberg/node\";")?;
    writeln!(buffer, "import {{ helpers }} from \"./helpers\";\n")?;
...
fn render_render_test_ts(fixture: &Fixture) -> Result<String> {
    ...
    writeln!(code, "        const documentPath = helpers.resolveDocument(\"{doc_path}\");")?;
    ...
    match render.mode.as_str() {
        ...
        "iterator" => {
            ...
            writeln!(
                code,
                "        const iter = new JsPdfPageIterator(documentPath{dpi_arg});"
            )?;
            writeln!(code, "        for (const pngData of iter) {{")?;
            writeln!(code, "            helpers.assertIsPng(pngData);")?;
            writeln!(code, "            pages.push(pngData);")?;
            writeln!(code, "        }}")?;
